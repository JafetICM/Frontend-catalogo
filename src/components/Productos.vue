    ...
    <div class="main-content">
      ...
      <div class="products-column">
        ...
        <div v-else-if="error" class="error-container">
          <p>{{ error }}</p>
          <button @click="fetchProductos" class="retry-button">Reintentar</button>
        </div>
  ...
  setup() {
    ...
    const fetchProductos = async () => {
      ...
      try {
        ...
        // Crear clave de cache única basada en la query
        const cacheKey = 'productos_' + JSON.stringify(query);
        const cacheTimestamp = localStorage.getItem(cacheKey + '_timestamp');
        const now = Date.now();
        
        // Cache válido por 10 minutos para productos
        if (cacheTimestamp && (now - parseInt(cacheTimestamp)) < 600000) {
        ...
        // Resetear página al cambiar resultados
        currentPage.value = 1;
      } catch (err) {
...
<style src="../assets/Productos.css"></style>
<style>
.page-btn {
  margin: 0 2px;
  padding: 0.5rem 1rem;
